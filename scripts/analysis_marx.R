...
# Columns: Name, Year (ignored), Political, Socialist, OriginalLanguage,
...
metadata_file <- here("data", "processed", "author_labels_marx.csv")
...
ngram_file <- here("data", "processed", "ngram_clean_marx.csv")
...
# Snapshot equivalent: Karl Marx, 1878-1932, English
snapshot_km_1878_1932 <- base_panel %>%
    filter(Year >= 1878, Year <= 1932) %>%
    normalize_panel()

cat("Snapshot (1878-1932):", n_distinct(snapshot_km_1878_1932$Name), "authors,",
    n_distinct(snapshot_km_1878_1932$Year), "years\n")
...
# ==============================================================================
    ...
    baseline_result <- tryCatch({
        run_synth(snapshot_km_1878_1932, "Karl Marx",
                  treatment_year = 1917,
                  pre_start=1878, pre_end=1916, post_end=1932)
    }, error = function(e) {
      ...
      if (!is.null(baseline_result)) {
          ...
          baseline_placebo <- tryCatch({
              run_placebo_test(snapshot_km_1878_1932, "Karl Marx",
                               treatment_year = 1917,
                               pre_start = 1878, pre_end = 1916, post_end = 1932)
          }, error = function(e) {
          ...
          save_synth_results(baseline_result, baseline_placebo, baseline_dir, label="_1917")
          cat("\nBaseline p-value (joint post std):",
              ifelse(!is.null(baseline_placebo), baseline_placebo$joint_post_std_p, "N/A"), "\n")
      }
